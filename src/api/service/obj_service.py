...
class ObjService:
    ...
    def get_air_info(self) -> AirDTO:
        config_scene = config.get_config_scene_dict()
        
        # Buscar el objeto aire en la configuración de la escena
        air_config = None
        for key in config_scene.keys():
            if "air" in key.lower():
                air_config = config_scene[key]
                break
        
        if air_config is None:
            raise HTTPException(status_code=404, detail="Configuración del aire no encontrada en la escena")

        return AirDTO(
            temperature=air_config.get("temperature"),
            attenuation=air_config.get("sigma_t"),
            wavelengths=config_scene.get("wavelengths"),
            num_bands=config_scene.get("num_bands")
        )
    
    def update_air_info(self, air_data: AirDTO) -> AirDTO:
        ...
        if air_config is None or air_key is None:
            raise HTTPException(status_code=404, detail="Configuración del aire no encontrada en la escena")
        
        # Actualizar la configuración del aire
        config_scene[air_key]["temperature"] = air_data.temperature
        config_scene[air_key]["sigma_t"] = air_data.attenuation
        config_scene["wavelengths"] = air_data.wavelengths
        config_scene["num_bands"] = air_data.num_bands

        # validar longitudes de atenuacion, wavelengths y num_bands
        if len(air_data.attenuation) != air_data.num_bands:
            raise HTTPException(status_code=400, detail="La longitud de la lista de atenuación debe ser igual a num_bands")
        
        if len(air_data.wavelengths) != air_data.num_bands:
            raise HTTPException(status_code=400, detail="La longitud de la lista de longitudes de onda debe ser igual a num_bands")

        # Validar temperatura
        if air_data.temperature <= 0:
            raise HTTPException(status_code=400, detail="La temperatura del aire debe ser superior a 0 (cero absoluto no permitido)")

        # Validar atenuación
        if not all(atten >= 0 for atten in air_data.attenuation):
            raise HTTPException(status_code=400, detail="Todos los valores de atenuación deben ser mayores o iguales a 0")
